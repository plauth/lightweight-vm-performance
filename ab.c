...
unsigned long long isqrt(long long number) {
  unsigned long long n  = 1;
  unsigned long long n1 = NEXT(n, number);
...
/*
   ...
   // -------------------------------------------------------------------------
   {
      ...
      "   of     ----------------------------\n"
      "Clients      min       ave       max      stdev      Time\n"
      "--------  --------  --------  --------  -------- --------------\n";
   ...
   for(i = FROM; i <= TO; i += STEP)
   {
	  ...
	  unsigned long long mean_diff = 0;
	  for(j = 0; j < ITER; j++) {
		  mean_diff += (nbr_array[j] - ave_rps) * (nbr_array[j] - ave_rps);
	  }
	  
	  int stdev = isqrt(mean_diff / ITER);
...
round_done:
      ...
      // ----------------------------------------------------------------------
      nbr = sprintf(buf, "%7d, %8d, %8d, %8d, %8d, %s  %s\n",
               i ? i : 1, min_rps,
               ave_rps, max_rps, stdev, cpu_ram_buf, tm_now());
